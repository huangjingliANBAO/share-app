	...
	<view>
		...
		<view v-else>
			<view class="header">
				...
				<view class="span">
					<button size="mini" type="primary" @click="sign" v-if="user.sign == 0">签到</button>
					<button size="mini" type="primary" @click="sign" v-else>已签到</button>
...
import { LOGIN_URL, USER_URL,SIGN_URL } from '@/utils/api';
export default {
	data() {
		return {
			user: null
	...
	methods: {
		...
		sign() {
			...
			const req = request(SIGN_URL, 'POST', userId);
			if(req.code == 200){
				uni.showModal({
					title:'签到成功',
					showCancel:true,
